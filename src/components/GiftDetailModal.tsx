  ...
  return (
    ...
    >
      <motion.div
        ...
        className={`relative flex max-h-[min(96vh,920px)] w-full flex-col overflow-hidden rounded-[1.35rem] border border-white/75 bg-white/95 shadow-soft backdrop-blur sm:max-h-[min(92vh,920px)] sm:rounded-[1.75rem] ${
          pixOpen ? 'max-w-6xl' : 'max-w-lg'
        }`}
      ...
      >
        ...
        >
          <div className="flex min-h-0 flex-col gap-4 overflow-y-auto p-4 pt-12 sm:gap-5 sm:p-8 sm:pt-14">
            <div className="w-full overflow-hidden rounded-2xl bg-linen ring-1 ring-inset ring-white/50">
              <div className="flex min-h-0 items-center justify-center p-2.5 sm:p-4">
                <img
                  src={gift.image}
                  alt={gift.name}
                  className="h-auto w-full max-h-[min(42vh,24rem)] max-w-full object-contain object-center brightness-[0.98] saturate-[0.95] sm:max-h-[min(48vh,28rem)]"
                />
              </div>
            </div>
          ...
          <AnimatePresence initial={false}>
            {pixOpen && (
              <motion.div
                ...
                className="min-h-0 overflow-y-auto border-t border-rose/20 bg-cream/35 p-4 sm:p-8 lg:max-h-[min(92vh,920px)] lg:border-t-0"
              >
                ...
                <p className="mt-3 text-[0.9rem] leading-6 text-truffle/72 sm:text-[0.95rem] sm:leading-7">
                  A lista funciona como uma referência de sugestões e valores. Contribua com o que fizer sentido pra você! É só abrir o app do banco, escanear o QR code ou
                  colar a chave. Se preferir cartão ou quiser combinar algo com a gente, a mamãe responde num toque no
                  WhatsApp.
                </p>
